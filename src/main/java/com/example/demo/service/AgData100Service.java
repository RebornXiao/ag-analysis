...
import com.example.demo.dao.AgDataMakeMoney100Repository;
import com.example.demo.model.AgDataMakeMoney100;
import jxl.Sheet;
import jxl.Workbook;
...
public class AgData100Service {
    ...
    @Transactional
    public void save(AgDataMakeMoney100 agData) {
        agDataMakeMoney100Repository.save(agData);
    }

    public List<AgDataMakeMoney100> getAll() {
        return (List<AgDataMakeMoney100>) agDataMakeMoney100Repository.findAll();
    }

    public long count() {
        return agDataMakeMoney100Repository.count();
    }

    public AgDataMakeMoney100 findOne(Integer id) {
        return agDataMakeMoney100Repository.findOne(id);
    }

    @Transactional
    public void save(List<AgDataMakeMoney100> agDataMakeMoney100List) {
        agDataMakeMoney100Repository.save(agDataMakeMoney100List);
    }
    ...
    public void test100Data() {
        ...
        int number = 0;
        int makeMoney = 0;
        List<AgDataMakeMoney100> moneyDayBig = new ArrayList<>();
        List<AgDataMakeMoney100> moneyByDb = getAll();
        for (int i = 0; i < 2500; i++) {
            AgDataMakeMoney100 agDataMakeMoney = moneyByDb.get(i);
...
//        System.out.println("赚钱大于30小于5000的天数：" + daySum+";分别是："+moneyDayBig.toString());
    ...
    private void stepNumber(AgDataMakeMoney100 AgDataMakeMoney100) {
        int day = AgDataMakeMoney100.getDayAll();
        if (day <= 10) {
            day10++;
            return;
        }
